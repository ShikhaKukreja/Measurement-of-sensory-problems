...
// $rowDrill = $drillResult->fetchAll();
    $sql1 = "SELECT Sum(feesPaid) as TOTALSCORE, otkey FROM feesPaid GROUP BY otkey";
    ...
    $arrDayOfMonth = array(
                "chart" => array(
                  "caption" => "Occupational Therapist Earnings",
                  "showValues" => "0",
                  "theme" => "zune",
                  "xAxisName"=> "Therapist ID",
                  "yAxisName"=> "Amount(In USD)",
                  )
               );
    $arrDayOfMonth["data"] = array();
...
foreach ($rowDrill as $key=>$val) {
    array_push($arrDayOfMonth["data"], array(
                "label" => $rowDrill[$key]['otkey'],
